...
export class InMemoryQueue<T> implements JobQueue<T> {
	...
	constructor(private readonly retry: RetryOptions = DEFAULT_RETRY) {}
	...
	private async run(payload: T): Promise<void> {
		...
		this.dead.push({ payload, attempts: this.retry.attempts, error: lastErr })
	}
...
export function createQueue<T>(
	opts: { redisUrl?: string; queueName?: string; retry?: RetryOptions } = {},
): JobQueue<T> {
	const queueName = opts.queueName ?? "belay-events"
	if (opts.redisUrl) {
		return new BullMQQueue<T>(opts.redisUrl, queueName, opts.retry ?? DEFAULT_RETRY)
	}
	return new InMemoryQueue<T>(opts.retry ?? DEFAULT_RETRY)
}
