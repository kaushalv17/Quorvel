...
export class PgStore implements Store {
	...
	async insertApiKey(rec: ApiKeyRecord): Promise<void> {
		await this.pool.query(
			`insert into api_keys (id, org_id, key_hash, key_prefix, name, created_at) values ($1,$2,$3,$4,$5,$6)`,
			[rec.id, rec.orgId, rec.keyHash, rec.keyPrefix, rec.name, rec.createdAt],
		)
	}
